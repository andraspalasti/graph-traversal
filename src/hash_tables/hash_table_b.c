...
static void set_value(HashTableB *self, char *key, bool val);
static void free_ht(HashTableB *self);
static bool *get_value(const HashTableB *self, const char *key);
...
HashTableB *init_ht_b(int size) {
    HashTableB *hash_table = malloc(sizeof(HashTableB));
 ...
 * @param key The key to search for
 * @return bool* The pointer to the value
 */
static bool *get_value(const HashTableB *self, const char *key) {
    int idx = hash(key) % self->size;
    Record *r = self->records[idx];
